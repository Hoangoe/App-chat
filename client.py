...
class GUI:
    ...
    def goAhead(self, username, room_id=0):
        ...
        time.sleep(0.1)
        self.server.send(str.encode(str(room_id)))
    ...
    def browseFile(self):
        self.filename = filedialog.askopenfilename(initialdir="/",
                                                   ...
                                                   filetypes=(("Text files",
                                                              ...
                                                              ("all files",
                                                               "*.*")))
        self.fileLocation.configure(text="Chọn file: " + self.filename)
    ...
    def sendFile(self):
        if self.filename != '':
            ...
            self.textCons.insert(tk.END, "You: Đã gửi "
        ...
        while True:
            try:
                ...
                if str(message) == "FILE":
                    ...
                    self.textCons.config(state=tk.DISABLED)
                    self.textCons.config(state=tk.NORMAL)
                    self.textCons.insert(tk.END, "Bạn đã nhận " + file_name + " từ " + str(send_user) + "\n\n")
                    self.textCons.config(state=tk.DISABLED)
                    self.textCons.see(tk.END)
