...
class Server:
    ...
    def clientThread(self, connection):
        ...
        if room_id not in self.rooms:
            connection.send("Đã tạo phòng chat mới".encode('utf-8'))
        else:
            connection.send("Chào mừng đến phòng chat!!!".encode('utf-8'))
            new_mem = str(user_id) + " đã vào phòng chat"
            self.broadcast(new_mem, connection, room_id)
